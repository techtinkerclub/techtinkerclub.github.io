...
const appWorked=read('assets/99club/games-app.js');
if(!appWorked.includes('function directWorkedExample(id)')||!appWorked.includes('renderDirectWorkedGuide'))fail('game-card-links','Guide button is not reusing the pack worked-example renderer');
...
if(!cardLinks.includes('/tools/99-club/games/?worked='))fail('game-card-links','Printable worked-example guide URL missing');
