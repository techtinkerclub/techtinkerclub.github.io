/* ===========================================================
   Tech Tinker Boss Battle — game.js (Arcade Build v6.2)
   - Matrix slowed
   - Bigger hit bounce
   - Boss HP bar never leaves a sliver; updates reliably
   - DnD submit hides after submit
   - If questions end and boss still has HP -> boss flees (roll-out)
   - Overlay hidden truly removed (no faint stage)
   =========================================================== */
...
(() => {
  ...
  const SKEY = 'ttcBossBattle_arcade_v6_2';
  ...
  function startMatrix(){
    ...
    function size(){
      ...
      w=c.width; h=c.height; cols=Math.max(1,Math.floor(w/16));
    ...
    let raf, t=0;
    function tick(){
      // Reduce frame rate to ~30fps; move slower
      t = (t+1)%2; if(t===1){ raf=requestAnimationFrame(tick); return; }
      g.fillStyle='rgba(8,12,26,0.18)'; g.fillRect(0,0,w,h);
      for(let i=0;i<cols;i++){
        const ch=chars[(Math.random()*chars.length)|0];
        const x=i*16, y=drops[i];
        g.fillStyle='rgba(140,255,170,0.8)'; g.fillText(ch,x,y);
        drops[i] = y>h ? 0 : y + (6 + Math.random()*6); /* slower fall */
      }
  ...
  function renderLevels(){
    screenLevels.innerHTML=''; const ids=Object.keys(DATA.weeks).sort((a,b)=>(+a)-(+b));
  ...
  function renderHUD(){
    ...
    const pct=(G.hp/G.hpMax)*100; // exact %
  ...
  function bossShowState(state){
    ...
    } else if(state==='dead'){ stageEl.classList.add('boss--dead'); }
  ...
  /* Finish when out of questions: defeat if hp==0, else flee */
  function finishOrFlee(){
    ...
    else { // boss flees to keep flow dynamic
      bossImg.classList.add('boss-roll-out'); bossImg.addEventListener('animationend',()=>showOverlay(false,true),{once:true});
    }
  }
