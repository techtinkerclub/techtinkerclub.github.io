/* ===========================================================
   Tech Tinker Boss Battle — game.js (Arcade Build v6.3)
   - Matrix: slower + bigger digits
   - Wrong answer FX: cabinet shake + red flash + boss lunge
   - Keeps v6.2 fixes (HP bar, DnD submit hide, roll-out/flee, etc.)
   =========================================================== */
...
(() => {
  ...
  const SKEY = 'ttcBossBattle_arcade_v6_3';
  ...
  /* Player HIT FX (wrong answer): cabinet shake + red flash + boss lunge */
  function playerHitFX(){
    stageEl.classList.add('player-hit','shake');
    bossImg.classList.remove('boss-lunge'); void bossImg.offsetWidth; // restart if running
  ...
  function settleAnswer(correct, explanation){
    if(correct){
      ...
      // NEW: cabinet shake + red flash + boss lunge
      playerHitFX();
    }
    renderHUD();
