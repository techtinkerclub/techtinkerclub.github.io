...
(function(global){
  ...
  function difficulty(){const value=getStored(DIFFICULTY_KEY,'standard');return ['easy','standard','challenge'].includes(value)?value:'standard';}
  function difficultyLabel(value){return value==='easy'?'Easy':value==='challenge'?'Challenge':'Standard';}
  ...
  function enhance(){
    ...
    const currentMode=mode(),currentCount=count(),currentDifficulty=difficulty();
    ...
    if(grid){
      const holder=document.createElement('div');holder.className='tt99-random-pack-fields wide';holder.innerHTML=`
        ...
        <label class="tt99-field ${currentMode==='random'?'':'random-only'}"><span>Difficulty</span><select id="games-random-difficulty"><option value="easy" ${currentDifficulty==='easy'?'selected':''}>Easy</option><option value="standard" ${currentDifficulty==='standard'?'selected':''}>Standard</option><option value="challenge" ${currentDifficulty==='challenge'?'selected':''}>Challenge</option></select><small>Applied consistently to every game chosen for a random pack.</small></label>
      ...
      holder.querySelector('#games-random-difficulty')?.addEventListener('change',e=>{setStored(DIFFICULTY_KEY,['easy','standard','challenge'].includes(e.target.value)?e.target.value:'standard');root.querySelector('#games-new-version')?.click();});
    ...
    if(currentMode==='random'){
      ...
      const text=firstStep?.querySelector('p');if(text)text.textContent=`Random mode is active. The app will choose a varied ${difficultyLabel(currentDifficulty).toLowerCase()} mix from every game that genuinely fits the selected years and topics.`;
      ...
      const packStep=buildCard.querySelector('.tt99-games-step p');if(packStep)packStep.textContent=`${difficultyLabel(currentDifficulty)} · ${currentCount} game activit${currentCount===1?'y':'ies'} · 2 per sheet · ${Math.ceil(currentCount/2)} pupil sheet${Math.ceil(currentCount/2)===1?'':'s'}.`;
    ...
    if(toolbar){const spans=toolbar.querySelectorAll(':scope > span');if(spans[0]){const sheets=Math.ceil(currentCount/2),prefix=currentMode==='random'?`Random ${difficultyLabel(currentDifficulty).toLowerCase()} mix`:'Selected games';spans[0].textContent=`${prefix} · ${currentCount} activit${currentCount===1?'y':'ies'} · ${sheets} sheet${sheets===1?'':'s'}`;}}
  }
