/* 99 Club Studio · Mobile Balance connected renderer fix v1.98
 * Re-registers only the Mobile Balance online adapter. The generator is unchanged.
 * Bars use equal arms with the suspension point at the true midpoint. Parent
 * bar movement is converted into endpoint coordinates, while hanging strings
 * remain vertical and child bars keep their own independent angle.
 */
(function(global){
'use strict';
...
function select(c,id,label,vals,labels={}){return `<label><span>${label}</span><select data-v198-opt="${id}">${vals.map(v=>`<option value="${v}" ${String(c[id])===String(v)?'selected':''}>${labels[v]||cap(v)}</option>`).join('')}</select></label>`;}
function bindOptions(root,c,onChange,norm){root.querySelectorAll('[data-v198-opt]').forEach(el=>el.addEventListener('change',()=>onChange(norm({...c,[el.dataset.v198Opt]:el.value}))));}
function numericPad(){return `<div class="tt99-v196-keypad" data-v198-pad>${[1,2,3,4,5,6,7,8,9].map(v=>`<button type="button" data-v198-digit="${v}">${v}</button>`).join('')}<button type="button" data-v198-back>⌫</button><button type="button" data-v198-digit="0">0</button><button type="button" data-v198-clear>Clear</button></div>`;}
...
function mobileSvg(p,state,feedback){
  ...
  function rec(n,cx,y,span){
    if(n.type==='group'){
      const gap=23,first=y+20,last=first+Math.max(0,n.count-1)*gap,bottom=last+13;
      let out=`<g class="tt99-mobile-leaf"><line class="string" x1="${fmt(cx)}" y1="${fmt(y)}" x2="${fmt(cx)}" y2="${fmt(bottom)}"/>`;
      for(let i=0;i<n.count;i++)out+=shapeSvg(n.shape,cx,first+i*gap,18,`shape s-${n.shape}`);
      return `${out}</g>`;
    }
    const id=barId++,arm=Math.max(48,Math.min(150,span*.28)),childY=y+72;
    let tilt=0;
    if(feedback&&branchKnown(n.left,vals)&&branchKnown(n.right,vals)){
      ...
      tilt=Math.max(-5,Math.min(5,(b-a)/m*9));
    }
    const left=endpoint(cx,y,-arm,tilt),right=endpoint(cx,y,arm,tilt),childSpan=Math.max(100,span*.5);
    return `<g class="tt99-mobile-subtree" data-bar="${id}"><line class="bar" x1="${fmt(left.x)}" y1="${fmt(left.y)}" x2="${fmt(right.x)}" y2="${fmt(right.y)}"/><circle class="tt99-mobile-pivot" cx="${fmt(cx)}" cy="${fmt(y)}" r="3"/><line class="string" x1="${fmt(left.x)}" y1="${fmt(left.y)}" x2="${fmt(left.x)}" y2="${fmt(childY)}"/>${rec(n.left,left.x,childY,childSpan)}<line class="string" x1="${fmt(right.x)}" y1="${fmt(right.y)}" x2="${fmt(right.x)}" y2="${fmt(childY)}"/>${rec(n.right,right.x,childY,childSpan)}</g>`;
  }
  const rootX=300,rootY=62,viewH=Math.max(255,depth(p.tree)*84+105),style=Object.entries(colors).map(([k,v])=>`--${k}:${v}`).join(';');
  ...
  return `${note}<svg class="tt99-mobile-svg" viewBox="0 0 600 ${viewH}" style="${style}">${top}${rec(p.tree,rootX,rootY,520)}</svg>`;
}
...
function mbMount(root,p,ctx){
  ...
  const stage=root.querySelector('[data-mobile-stage]'),boxes=[...root.querySelectorAll('[data-mobile-answer]')],pad=root.querySelector('[data-v198-pad]');
  ...
  function click(e){const b=e.target.closest('[data-mobile-answer]');if(b&&!finished){selected=b.dataset.mobileAnswer;hint=null;render();return;}const d=e.target.closest('[data-v198-digit]');if(d)return commit(appendDigit(state[selected],d.dataset.v198Digit));if(e.target.closest('[data-v198-back]'))return commit(backspace(state[selected]));if(e.target.closest('[data-v198-clear]'))return commit(null);}
...
Play.registerAdapter('mobilebalance',{
  ...
  howText:'The suspension point is in the middle of every bar, so the left and right sides carry equal weight. Repeated shapes mean repeated copies of the same value. On nested mobiles, solve a lower bar first, then use the total weight of that whole branch higher up. If a number is shown in the top circle, it is the total weight of the whole mobile.',
