...
(function(global){
  ...
  function drawEquationCrossgrid(page,a,answers,x,y,w,h,index){
    const top=activityFrame(page,x,y,w,h,index,a);drawWrapped(page,x+12,top,a.instruction,w-24,6.8,{color:MUTED,maxLines:2});const solution=a.solutionGrid||[],shown=answers?solution:(a.displayGrid||solution),hidden=new Set(a.hiddenKeys||[]),bodyY=top+28,bodyH=h-(bodyY-y)-14,gridSize=Math.min(w-90,bodyH,285),cell=gridSize/5,gx=x+w/2-gridSize/2,gy=bodyY+Math.max(0,(bodyH-gridSize)/2);
    for(let r=0;r<5;r++)for(let c=0;c<5;c++){const sol=solution?.[r]?.[c],v=shown?.[r]?.[c],key=`${r}:${c}`,cx=gx+c*cell,cy=gy+r*cell,block=sol==='#',answerFill=answers&&hidden.has(key);page.rect(cx,cy,cell,cell,{fill:block?[239,244,244]:answerFill?HIT:WHITE,stroke:block?[218,226,227]:[112,137,142],width:block?.35:.65});if(block||v==null)continue;const text=v==='-'?'−':String(v),isOp=['+','-','×','÷','='].includes(sol),fs=isOp?Math.max(7,Math.min(13,cell*.28)):Math.max(7.2,Math.min(12.5,cell*.27));diagramText(page,cx+cell/2,cy+cell*.63,clean(text),fs,{bold:true,color:answerFill?TEAL:INK});}
    page.rect(gx,gy,gridSize,gridSize,{fill:null,stroke:[80,106,111],width:.9});
  }
