...
Games & Puzzles uses a shared game-engine/content-provider architecture. The current line provides Word Search, Number Pyramid, Crossword, Magic Squares and Sudoku / Latin Squares, a 591-entry curated primary-maths vocabulary catalogue, per-engine settings, richer optional front-of-pack worked examples and selective preview replacement. Magic Squares includes missing-value, check-if-magic, error-repair and transform variants with 3×3/4×4, shifted/scaled/decimal number patterns and deterministic validation. Sudoku supports 4×4, 6×6 and full 9×9 grids with uniqueness checking; Latin Squares remain available as the row/column-only derivative. Crossword clues automatically show answer-length enumeration such as (5, 5). A whole-game ↻ regenerates only that activity using the same engine/settings. Word Search also exposes a clue-level ↻ that swaps only one term/definition pair while preserving the other selected vocabulary. Word direction can remain automatic by difficulty or be overridden independently. Teacher-added vocabulary remains browser-local/importable/exportable and never edits the curated catalogue.
