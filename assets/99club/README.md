...
3. **Maths Games & Puzzles** at `/tools/99-club/games/` — printable independent practice built from reusable game engines and maths-content providers. Current engines are Word Search, Number Pyramid, Crossword and Magic Squares, with local teacher vocabulary support for the vocabulary-based games.
...
- Adds reusable game engines with year-range, topic, per-engine difficulty/options, sheet-count and activities-per-sheet controls.
...
Games & Puzzles uses a shared game-engine/content-provider architecture. v1.25 provides Word Search, Number Pyramid, Crossword and Magic Squares, a 591-entry curated primary-maths vocabulary catalogue, per-engine settings, richer optional front-of-pack worked examples and selective preview replacement. Magic Squares includes missing-value, magic-total, error-repair and transform variants with 3×3/4×4, shifted/scaled/decimal number patterns and deterministic validation. Crossword clues automatically show answer-length enumeration such as (5, 5). A whole-game ↻ regenerates only that activity using the same engine/settings. Word Search also exposes a clue-level ↻ that swaps only one term/definition pair while preserving the other selected vocabulary. Word direction can remain automatic by difficulty or be overridden independently. Teacher-added vocabulary remains browser-local/importable/exportable and never edits the curated catalogue.
