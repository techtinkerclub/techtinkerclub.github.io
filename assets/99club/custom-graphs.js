...
(function(global){
  ...
  function footprintHeight(item,answers,orientation){
    ...
    return (map[fp]||map.L)+tableBonus;
  }
  ...
  function renderBar(C,x,y,w,h,v,answers){
    ...
    const fills=[[76,145,140],[221,169,70],[117,132,143]];
    cats.forEach((cat,j)=>{
      series.forEach((s,k)=>{const hidden=!answers&&(v.hideAllBars||v.hiddenBars?.includes(j));if(hidden)return;const val=Number(s.values[j]||0),yy=yp(val),bx=left+j*groupW+(groupW-usable)/2+k*barW+1,bw=Math.max(3,barW-2),bt=Math.min(zero,yy),bh=Math.max(1,Math.abs(zero-yy));C.rect(bx,bt,bw,bh,{fill:fills[k%fills.length],stroke:[50,80,80],width:.4});});
  ...
  function renderLine(C,x,y,w,h,v,answers){
    ...
    const colors=[[35,122,116],[202,132,45],[99,111,122]];
  ...
  function PdfCanvas(page){return {text:(...a)=>page.text(...a),line:(...a)=>page.line(...a),rect:(...a)=>page.rect(...a),image:(...a)=>page.image(...a),replaceButton:()=>{}};}
  ...
  function polishCustomUi(){
    ...
    const intro=root.querySelector('.tt99-custom-intro-note');if(intro){intro.innerHTML='<strong>Visual maths has started.</strong><span>Bar charts, Year 4 time graphs and Year 5–6 line graphs now use deterministic generated visuals. Other diagram-heavy topics are still added only when they have dedicated validated renderers.</span>';}
  ...
  if(typeof document!=='undefined'){
    const root=document.getElementById('tt99-root');if(root){new MutationObserver(()=>polishCustomUi()).observe(root,{childList:true,subtree:true});setTimeout(polishCustomUi,0);}
  }

  global.TT99CustomGraphs={VERSION,BAR_CATALOGUE,LINE_CATALOGUE,ELIGIBLE,GRAPH_FAMILIES,IMPLEMENTED_BAR:[...IMPLEMENTED_BAR],IMPLEMENTED_LINE:[...IMPLEMENTED_LINE],allowedTypes,graphPool,packQuestions,footprintHeight,renderVisualPreview,buildVisualDocument};
